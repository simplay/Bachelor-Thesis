...
\begin{table}[H]
  \begin{tabular}{@{}lll@{}}
    ...
    \textbf{Procedure:} & $getMaxIntensGridPointsOf(matrix)$ get indices for largest intensivity in matrix \\
...
\begin{boxedminipage}{1.0\textwidth}
  \begin{algorithmic}[1]
    \ForAll{$\lambda_k \in \Lambda$}
      \State $(\widetilde{\lambda}, \widetilde{\alpha}) = getMaxIntensGridPointsOf(R)$
      \Comment{$\widetilde{\lambda}$ $\equiv$ wavelength, $\widetilde{\alpha}$ $\equiv$ viewing angle of max. R}
      ...
      \State $plot(\lambda_k, \widetilde{\alpha}_{r_k})$
      \Comment{graph resulting by our BRDF model}
      \State $plot(\lambda_k, \theta_{r_k})$
