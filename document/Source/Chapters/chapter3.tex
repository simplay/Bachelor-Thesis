...
Within this loop happens the uniform sampling along wavelength-space. At line 5: $ColorWeights(\lambda)$ computes the color weights for the current wavelength by bilinear interpolation between the two closest given $CIE_{XYZ}$ color weights for our current wavelength $\lambda$ stored in a external weights-table. At line 6: $lookupCoord(u, v, \lambda)$ computes the current coordinate for the texture lookup in our precomputed ifft2 images - See $\ref{eq:scalelook}$. 

\myparagraph{From line 9 to 24:} 
Within this loop happens the Taylor series approximation till a predefined upper bound, denoted as $T$.
...
$gaussWeightOf(dist)$ computes the weights $~\eqref{eq:gaussweight}$ from the distance between the current pixel's coordinates and the current neighbor's position in texture space - See $\ref{eq:gaussianwindow}$. 

$rescaledFourierValueAt(pos)$ rescales the current computed value by the precomputed extrema values since all image values are scaled between 0 and 1. 
