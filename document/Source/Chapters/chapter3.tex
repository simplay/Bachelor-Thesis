...
In oder to render structural colors due light diffracted on a grating, our fragment shader expects being provided by the precompute height field DFT terms as explain in section $\ref{sec:precompmatlabfourierimages}$ and by the processed output of the verte shading process, which is, the light- and viewing-direction vectors $\omega_i$ and $\omega_r$ used in our model described in section $\ref{sec:spectralrendering}$.



from section $d$ and the vertex shader output from section.
