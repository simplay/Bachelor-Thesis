...
Vertex shaders are special functions used to manipulate the vertex data by usi
ng mathematical
operations. Basic vertices are stored with an X, Y,
and
Z position. Another addition is the color of a vertex.
Using vertex shaders you can manipulate the position and colors, changing only their values and not the way
the
ir
data is stored. Color
s and position are very basic but are whats needed to build up to vertex shading
examples that can do things like manipulate the vertex position to create more fuid animations.


Im heutigen Artikel werden wir uns mit den Grundlagen der GLSL Shader Programmierung vertraut machen. Ausgangspunkt für unsere Betrachtungen ist das OpenGL
Vertex Shaders enable an unlimited palette of visual effects that can be rendered in real time.
 output: Im Vertex Shader berechnete Texturkoordinaten, die im Fragment Shader berechnete Pixelfarbe usw.
...
Vertex-Shader dient dazu, die Geometrie einer Szene zu manipulieren
 Koordinaten der Vertices der Oberflächen transformiert (mesh)
 So lässt sich die Form von Objekten (nur bereits bestehende Obj) beeinflussen, was sich wiederum auch in ihrer Beleuchtung niederschlägt
 Vertex-Shadern sind Effekte wie Vertex Lighting, Terrain-/Objektdeformation, Wasserwellen oder Fischaugenobjektiv möglich
 Der Shader hat dabei nur Zugriff auf den gerade behandelten Vertex (inkl. seiner Texturkoordinaten, Normalen und sonstiger übergebener Daten), aber nicht etwa auf Nachbar-Vertices, die Topologie oder Ähnliches.
...
\begin{figure}[H]
  ...
  \caption[Camera Coordinate System]{Camera coordinate system where its origin defines the center of projection of camera}
...
\begin{figure}[H]
  ...
  \caption[Camera Matrix]{Illustration of involved components in order to construct the camera matrix. We introduce some helper vectors $z_c = \frac{e-d}{||e-d||}$, $x_c = \frac{up \times z_c}{||up \times z_c||} $ and $z_c \times x_c$ for the actual construction of the camera matrix}
...
\begin{figure}[H]
  ...
  \caption[Rays of a Directional Light]{For a directional light source all light rays are in parallel.}
