...
\section{Definitions}
In order to prepare the reader for the upcoming derivations, we give a brief definition and introduction for most important terms required to follow. Concepts from physics, such as diffraction and wave theory, are based on book Tipler $\cite{tipler}$. Most of the computer graphics theory is either based on the book $\cite{fundcg}$ or from previous achieved knowledge during my academic career. The mathematical concepts and definitions are either based based on the following two books Analysis 3 by Otto Forster $\cite{ana3}$ and or the book written by H. Bartsch $\cite{fosa}$ or Wikipedia, whereat this would be clearly be cited.

\subsection{Diffraction}
\label{diffractiondeff}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.3]{background/solidangle.png}
