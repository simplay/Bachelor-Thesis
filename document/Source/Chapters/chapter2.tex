...
Stam formulates for a hightfield auxilary function 
...
where the scalar $w$ is the third component of ~\ref{eq:uvw} and can be written as $w = -(cos(\theta_i)+cos(\theta_r))$ using spherical coordinates. The scalar $k=\frac{2\pi}{\lambda}$ represent the wavenumber.
...
Where $BRDF_{\lambda}(\omega_i, \omega_r)$ is the a BRDF $\ref{brdf}$ with wavelength $\lambda$, $A$ is the surface area,
$P(x,y)$ is the Fourier Transform (FT) of the function $p(x,y)$ ~\ref{eq:px}.
G is the geometry term which is 
