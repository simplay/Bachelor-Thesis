...
\caption[BRDF Map of FLSS approach on Gratings]{BRDF maps for different patches}
...
The Figures $\ref{fig:pqblaze}$, $\ref{fig:pqelaphe}$, $\ref{fig:pqxeno}$  show a comparison of the BRDF maps produced by the FLSS approach (on the left) and the PQ shading approach (on the right) applied on all our patches. For Blazed grating, as already mentioned, we notice that both approaches, FLSS and PQ, resemble each other. We also notice that for PQ map, the first order diffraction color contribution is spread. For Elaphe and Xenopeltis grating we notice similar shaped BRDF patterns, even when the angle of light varies, but nevertheless, they also contain some artifacts. For the PQ approach, this also holds true when we do not apply we a sinc-interpolation like shown in figure $\ref{fig:pqBlaze}$.
...
\caption{Blazed grating with periodicity of $2.5 \mu m$: Different $\sigma_s$}
...
% NEW
In section $\ref{sec:taylorapproximation}$

For all our gratings we observed visually a convergence of our shading approaches when using $N \geq 39$ DFT terms. Furthermore, for a Blazed grating it satisfied to use only $N \geq 7$ and for an Elapge grating only $N \geq 9$ DFT terms. Notice, that these values of terms were determined empirically by try and error. 

However, by using taylor error term estimates, like introduced in the appendix $APPENDIXSECTION$, we can derive an upper bound for $N$. But, since this computation is dependent on many aspects, such as grating spacing, pixel-width correspondance, used lambda space for sampling and so on, it is usually way simpler to determine empirically values for N 

In general it is rather hard to say for what value of $N$ we have visual convergence since in algorithm $\ref{alg:matlabprecomp}$ we compute the inverse FFT of $power(1j*patchImg, t)$ which is equivalent to $iFFT(patchImg)^n \cdot i^n$. Since we multiply this by $i^n$ there are four possible series where each converges within its own convergence radius.
% END NEW


% UNSAFE
In section $\ref{sec:taylorapproximation}$ we already provided a proof for the convergence for a specific geometrical set up which could be applied easily to another setup. The higher the value of $N$ the less the BRDF map changes. For the Blazed grating, we do not see much changing patterns for $N \geq 7$ and for the Elaphe surface grating $N \geq 9$. In general it is rather hard to say for what value of $N$ we have visual convergence since in algorithm $\ref{alg:matlabprecomp}$ we compute the inverse FFT of $power(1j*patchImg, t)$ which is equivalent to $iFFT(patchImg)^n \cdot i^n$. Since we multiply this by $i^n$ there are four possible series where each converges within its own convergence radius.
% END UNSAFE
