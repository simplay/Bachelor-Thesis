...
apparent bending of waves due to huygens principle
encounters an obstacles which is comparable in size to wave's wavelength


When a propagating wavefront is partially occluded by an obstacle, the wave is not only moving in the direction along its propagation, but is also bent along the boundaries of the obstacle. This phenomenon is called diffraction an is a direct cause of huygen's principle.

the effect of diffraction is generally most expressed for waves whose wavelength is roughly similar in size to the dimension of the occluding object. Conversely, if the wavelength is hardly similar in size, then there is almost no diffraction at all. 
