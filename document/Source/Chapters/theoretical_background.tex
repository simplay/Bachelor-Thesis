...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.9]{background/waveschema.png}
  \caption[sinewave]{Simplified, one dimensionaly real valued wave function$\footnotemark$, giving an idea about the properties of a wave.}
  \label{fig:wavebasics}
\end{figure}
\footnotetext{Modified, schematically sine-wave}
...
\begin{description}
  ...
  \item[Amplitude:] There are two possible interprations. First, it is a measure of the height from the equilibrium point tp the heighest point of a crest on the wave. For light waves it is a relative measure of intensity or brightnes to other lught waves of the same wavelength. And secondly, it can be interpreted as a measure how much energy a wave carries wherate the greater the absoulte aplitute value, the bigger the amount of energy being carried.
  \item[Frequency:] Is a measure of the number of waves passing through a point in the propagation medium during a certain time.
  \item[Phase:] describes either the offset of a wave's initial position or the relative displacement between or among waves having the same frequency. Two waves two waves with same frequency are denoted by being in phase if they have the same phase. This means they line up everywhere. 
\end{description}

A geometrical property of a wave is the wavefront. This is either a surface or line along the path of wave motion on which the disturbance at every point has the same phase. Note that for electromagnetic waves, the phase is the definition of poisition of a point in time on a wavefront cycle whereat a complete cycle is defined as being 360 degrees.
