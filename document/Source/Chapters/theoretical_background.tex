...
\begin{figure}[H]
  ...
  \caption[Irradiance]{Irradiance is the summed up radiance over all directions}
...
\begin{itemize}
  ...
  \item Temporal coherence examines the ability of how well a wave will interfere with itself at different moments in time. Mathematically, this kind of coherence is computed by averaging the measured correlation between the value of the wave and the delayed version of itself at different pairs of time. The Coherence time denotes the time for which the propagating wave is coherent and we therefore can predict its phase using the correlation function. The distance a wave has traveled during the coherence time is denoted as the coherence length.
\end{itemize}
