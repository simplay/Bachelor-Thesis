...
The direction vector of the secondary wavelet can be computet by taking the difference between the incident and viewing direction like in equation $\ref{eq:uvw}$:
\begin{equation}
...
auxillary function $\Phi$ which models the phase of a wave from the provided height field.
\begin{equation}
  \Phi(x,y) = \frac{2 \pi}{\lambda} w h(x,y) 
\label{eq:heightfieldphase}
\end{equation}

Then, a secondary wavelet $p$ on emitted off from the surface will looke like:
\begin{equation}
  p(x,y) = e^{i\Phi(x,y)} 
\label{eq:px}
\end{equation}

Then, following the idea presented for figure $\ref{fig:meaningofstamsapproach}$ and the mathematical steps in the appendix, we get the following brdf for the total contribution of all secondary sources:
\begin{equation} 
