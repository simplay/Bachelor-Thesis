...
A wave is the spatial propagating variation or rather disturbation or also an oscilation of a location-and time-dependent physical quantity. In Mathematics, when talking about a Wave, we are refering to the so called wavefunction  $y(x,t) = A sin(wt - kx)$ which is a solution for the so called wave-equation. In general, this function depends on the location $x$ and in time $t$.
