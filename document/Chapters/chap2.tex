...
We assume, that our incident light is a directional light source like sun-light and therefore its radiance is given as $L_{\lambda}(\omega)=I(\lambda)\delta(\omega-\omega_i)$ where $I(\lambda)$ is the intensity of the relative spectral power for the wavelength $\lambda$. Thus we get for our the brdf formulation:
...
where $\omega_i$ is the solid angle for the incoming light, $\theta{_i}$ is the angle of incidence,
$\omega_r$ is the solid angle for the reflected light, $\lambda$ wavelength, $\Omega$ is the hemisphre we of integration for the incomming light.
Radiance reflected by given surface in given direction:
$L_{\lambda}(\omega_i)$ is the incomming radiance, $L_{\lambda}(\omega_r)$ is the reflected radiance

For the $BRDF(\omega_i, \omega_r)$ we are going to use the formulation dervied by Stam described above which looks like this using the fact that wavenumber $k=\frac{2\pi}{\lambda}$:
