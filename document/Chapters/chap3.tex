...
rendering engine (jrtr) using a low-level api openGL 
assigning GLSL shaders communication, set-up by jogl
assigning scene data to rendering pipeline - to shaders
set up buffers which will be used by application programs canvas in order to be rendered.
definition of camera, perspective projection, scene graph, lights
resource management
flush,allocate, prestore buffers
rendering scene graph
optimizations like view-frustum-culling: discard renderItems outside view-frustum
assign textures
allows to render by own defined vertex- and fragment-shaders
...
application prorgam: 
set up scene-data: camera, perspective projection, materials, geometries
packing into container, passed to jrt
