...
void main() {
	...
	vec3 camNormal = normalize((modelview*vec4(normal,1)).xyz);
	vec3 camTangent = normalize((modelview*vec4(tangent,1)).xyz);
	...
	float alpha = acos( dot(normalize(-_k1), normalize(camNormal)) );
	float teta = (2*PI - alpha);
	...
	for(int iter = 0; iter < MaxIter; iter++){
		...
		if(coords.x < 0.0 || coords.y < 0.0 || coords.x > 1.0 || coords.y > 1.0) mayRun = false;
