...
public class VertexData {
	...
	public float[] getDataBySchematic(Semantic semantic){
		float[] data = null;
		for(VertexElement e : vertexElements)
			if(e.semantic.equals(semantic)) data = e.data;
	...
	//TODO write me
	public void replaceElement(float f[], Semantic s){
		for(int k = 0; k < vertexElements.size(); k++){
			VertexElement element = vertexElements.get(k);
			if(element.getSemantic() == s) element.data = f;
		}
	}
	...
	public void addElement(float f[], Semantic s, int i){
		if(f.length==n*i) {
			...
			if(s == Semantic.POSITION) vertexElements.addLast(vertexElement);
			else vertexElements.addFirst(vertexElement);
		}	
	}
